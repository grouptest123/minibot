...
def _feature_flags(evidences: list[Evidence]) -> list[str]:
    features: list[str] = []
    if any(item.source == "video" and "离岗" in item.summary for item in evidences):
        features.append("人员离岗")
    if any(item.source == "video" and "静止" in item.summary for item in evidences):
        features.append("人员静止过久")
    if any(item.source == "video" and ("遮挡" in item.summary or "不可用" in item.summary) for item in evidences):
        features.append("视频不可用")
    if any(item.source == "metric" and item.detail.get("cpu", 0) >= 90 for item in evidences):
        features.append("CPU越阈值")
    if any(item.source == "metric" and item.detail.get("latency_ms", 0) >= 1500 for item in evidences):
        features.append("延迟越阈值")
...
def build_observation_windows() -> list[ObservationWindow]:
    ...
    for metric in get_metrics():
        ...
        summary = f"cpu={metric['cpu']} memory={metric['memory']} latency={metric['latency_ms']}"
        buckets[minute].append(Evidence(source="metric", timestamp=metric["timestamp"], summary=summary, detail=metric))
    ...
    for minute, evidences in sorted(buckets.items()):
        features = _feature_flags(evidences)
        risk_level = "low"
        if len(features) >= 4:
            risk_level = "high"
        elif len(features) >= 2:
